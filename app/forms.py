...
class EditProfileForm(FlaskForm):
    ...
    def __init__(self, original_username, *args, **kwargs):
        super(EditProfileForm, self).__init__(*args, **kwargs)  # passes this call to the parent class, plus its args
        self.original_username = original_username

    def validate_username(self, username):
        # only validate for uniqueness if the username was changed
...
class PostForm(FlaskForm):
    post = TextAreaField(_l('What\'s on your mind?'), validators=[DataRequired(), Length(min=1, max=140)], render_kw={'autofocus': True})
    submit = SubmitField(_l('Submit'))
